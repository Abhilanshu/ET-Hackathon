...
// Download the PNG directly into the project directory for the user
https.get(`https://mermaid.ink/img/${data}`, (response) => {
    if (response.statusCode !== 200) {
        console.error("Failed with status code: " + response.statusCode);
        return;
    }
    const file = fs.createWriteStream('C:\\Users\\ASUS\\OneDrive\\Desktop\\Antigravity\\et-finance-mentor\\mentor_ai_architecture.png');
    response.pipe(file);
    file.on('finish', () => {
        file.close();
        console.log('Image successfully downloaded to: C:\\Users\\ASUS\\OneDrive\\Desktop\\Antigravity\\et-finance-mentor\\mentor_ai_architecture.png');
    });
}).on('error', (err) => {
